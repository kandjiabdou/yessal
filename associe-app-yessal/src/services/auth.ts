...
export interface SiteSessionInfo {
  activeAssociesCount: number;
  activeAssocieIds: number[];
...
class AuthService {
  ...
  /**
   * Récupère l'ID du site de lavage principal du associe connecté
  ...
  static async updateAssocieSite(siteId: number): Promise<boolean> {
    try {
      const user = this.getUser();
      if (!user) {
  ...
  /**
   * Démarre ou met à jour une session de travail pour le associe connecté
  ...
  static async setWorkSession(siteId: number | null): Promise<boolean> {
    try {
      ...
      const response = await apiClient.post<{ 
        success: boolean; 
        data: { associeId: number; currentSessionSiteId: number | null } 
      }>(
        `/associes/${user.id}/work-session`,
        { siteId }
      );
  ...
  /**
   * Récupère la session de travail actuelle du associe connecté
   */
  ...
  /**
   * Met à jour l'activité du associe pour éviter l'expiration de session
   */
