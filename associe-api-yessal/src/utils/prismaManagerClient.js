...
const prismaManager = new PrismaClient({
  datasources: {
    db: {
      url:
        process.env.DATABASE_MANAGER_URL ||
        "mysql://root:admin@localhost:3306/yessal",
    },
  },
});
...
prismaManager.abonnementpremiummensuel = {
  async aggregate(options = {}) {
    ...
    query +=
      selectParts.join(", ") + " FROM abonnementpremiummensuel WHERE 1=1";
    ...
    // ⚠️ AJOUT: Gérer le filtre createdAt
    if (where.createdAt) {
      ...
      if (where.createdAt.lt) {
        query += " AND createdAt < ?";
        params.push(where.createdAt.lt);
      }
    }
