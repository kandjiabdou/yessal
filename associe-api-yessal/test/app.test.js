...
describe('App configuration and middleware', () => {
  describe('CORS configuration', () => {
    ...
    test('allows development origins using OPTIONS', (done) => {
      ...
      request(app)
        .options('/api/auth')
        .set('Origin', 'http://localhost:4550')
        .expect((res) => {
          expect(res.headers['access-control-allow-origin']).toBe('http://localhost:4550');
        })
        .end((err) => {
    ...
    test('allows production origins when NODE_ENV is production using OPTIONS', (done) => {
      ...
      request(app)
        .options('/api/auth')
        .set('Origin', 'https://associe.yessal.sn')
        .expect((res) => {
          expect(res.headers['access-control-allow-origin']).toBe('https://associe.yessal.sn');
        })
        .end((err) => {
          process.env.NODE_ENV = originalEnv;
          done(err);
