...
// --- Compatibility: existing combined endpoint ---
const getDashboardData = async (req, res, next) => {
  try {
    // call both handlers internally to keep compatibility with existing clients
    // note: we build their internal calls rather than HTTP requests
    const fakeReqToday = { params: req.params, query: {} };
    const fakeReqPeriod = { params: req.params, query: req.query };

    // reuse functions: getTodayData and getPeriodData return response via res, so we'll duplicate logic here
    // Instead, compute directly by calling the internal logic used above
    const { siteId } = req.params;
    const { period = 'week', offset = '0' } = req.query;
    const siteIdInt = parseInt(siteId);
    const offsetInt = parseInt(offset);
    ...
    // get today
    const { startOfToday } = computePeriodRange('day', 0);
    const [todayOrders, recentOrders, periodOrders, todayAbonnements, periodAbonnements, todayNewClientsCount, periodNewClientsCount] = await Promise.all([
      prisma.commande.findMany({ where: { siteLavageId: siteIdInt, flag: true, dateHeureCommande: { gte: startOfToday } }, select: { id: true, prixPaye: true, masseVerifieeKg: true, masseClientIndicativeKg: true, estEnLivraison: true, statut: true } }),
      prisma.commande.findMany({ where: { siteLavageId: siteIdInt, flag: true }, include: { clientUser: { select: { nom: true, prenom: true } }, clientInvite: { select: { nom: true } } }, orderBy: { dateHeureCommande: 'desc' }, take: 5 }),
      (async () => {
        const { periodStart, periodEnd } = computePeriodRange(period, offsetInt);
        return prisma.commande.findMany({ where: { siteLavageId: siteIdInt, flag: true, dateHeureCommande: { gte: periodStart, lt: periodEnd } }, select: { id: true, prixPaye: true, masseVerifieeKg: true, masseClientIndicativeKg: true, estEnLivraison: true, statut: true } });
      })(),
      prisma.abonnementpremiummensuel.findMany({ where: { createdAt: { gte: startOfToday } }, select: { montant: true } }),
      (async () => {
        const { periodStart, periodEnd } = computePeriodRange(period, offsetInt);
        return prisma.abonnementpremiummensuel.findMany({ where: { createdAt: { gte: periodStart, lt: periodEnd } }, select: { montant: true } });
      })(),
      prisma.user.count({ where: { createdAt: { gte: startOfToday }, flag: true } }),
      (async () => {
        const { periodStart, periodEnd } = computePeriodRange(period, offsetInt);
        return prisma.user.count({ where: { createdAt: { gte: periodStart, lt: periodEnd }, flag: true } });
      })()
    ]);
    ...
    const todayStats = {
      ...
      totalPoidsKg: todayOrders.reduce((sum, o) => sum + (o.masseVerifieeKg ?? o.masseClientIndicativeKg ?? 0), 0),
      totalLivraisons: todayOrders.filter(o => o.estEnLivraison && o.statut === 'Livre').length,
      totalAbonnementsCreated: todayAbonnements.length,
      totalAbonnementMontant: todayAbonnements.reduce((s, a) => s + (a.montant || 0), 0),
      totalNewClients: todayNewClientsCount
    };
    if (todayStats.totalAbonnementMontant) todayStats.totalRevenue += todayStats.totalAbonnementMontant;

    const periodOrdersArr = periodOrders;
    const periodStats = {
      totalCommandes: periodOrdersArr.length,
      totalRevenue: periodOrdersArr.reduce((s, o) => s + (o.prixPaye || 0), 0),
      totalPoidsKg: periodOrdersArr.reduce((s, o) => s + (o.masseVerifieeKg ?? o.masseClientIndicativeKg ?? 0), 0),
      totalLivraisons: periodOrdersArr.filter(o => o.estEnLivraison && o.statut === 'Livre').length,
      totalAbonnementsCreated: periodAbonnements.length,
      totalAbonnementMontant: periodAbonnements.reduce((s, a) => s + (a.montant || 0), 0),
      totalNewClients: periodNewClientsCount
    };
    if (periodStats.totalAbonnementMontant) periodStats.totalRevenue += periodStats.totalAbonnementMontant;

    // Abonnements en cours for month
    if (period === 'month') {
      const { periodStart } = computePeriodRange(period, offsetInt);
      const month = periodStart.getMonth() + 1;
      const year = periodStart.getFullYear();
      periodStats.totalAbonnementsEnCours = await prisma.abonnementpremiummensuel.count({ where: { annee: year, mois: month, flag: true } });
    } else {
    ...
    const formattedRecentOrders = recentOrders.map(order => {
      ...
      return { id: order.id, clientName, prixPaye: order.prixPaye || 0, masseClientIndicativeKg: order.masseClientIndicativeKg, statut: order.statut, dateHeureCommande: order.dateHeureCommande.toISOString() };
    });

    // build periodInfo
    const { periodStart, periodEnd } = computePeriodRange(period, offsetInt);

    res.status(200).json({ success: true, data: { todayStats, periodStats, recentOrders: formattedRecentOrders, siteName: site.nom, periodInfo: { startDate: periodStart.toISOString(), endDate: periodEnd.toISOString(), offset: offsetInt, period, isCurrentPeriod: offsetInt === 0 } } });
  } catch (error) {
    next(error);
  }
};
