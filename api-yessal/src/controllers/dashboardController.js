...
const getDashboardData = async (req, res, next) => {
  try {
    ...
    const getCurrentWeekStart = (offset = 0) => {
      const today = new Date(); // ou ta variable "today"
      const date = new Date(today);
      const dayOfWeek = date.getDay(); // 0 = dimanche, 1 = lundi, ..., 6 = samedi

      // Si c'est samedi, on recule de 0 jour, sinon on recule pour atteindre le samedi précédent
    ...
    const todayStats = {
      ...
      totalRevenue: todayOrders.reduce(
        (sum, order) => sum + (order.prixPaye || 0),
        0
      ),
      totalPoidsKg: todayOrders.reduce(
        (sum, order) =>
          sum + (order.masseVerifieeKg || order.masseClientIndicativeKg),
        0
      ),
      totalLivraisons: todayOrders.filter(
        (order) => order.estEnLivraison && order.statut === "Livre"
      ).length,
    };
    ...
    const weekStats = {
      ...
      totalRevenue: weekOrders.reduce(
        (sum, order) => sum + (order.prixPaye || 0),
        0
      ),
      totalPoidsKg: weekOrders.reduce(
        (sum, order) =>
          sum + (order.masseVerifieeKg || order.masseClientIndicativeKg),
        0
      ),
      totalLivraisons: weekOrders.filter(
        (order) => order.estEnLivraison && order.statut === "Livre"
      ).length,
    };
